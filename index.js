...
const indexTemplate = ejs.compile(
  fs.readFileSync("templates/index.ejs", { encoding: "utf8" })
);
const manifest = JSON.parse(
  fs.readFileSync("dist/manifest.json", { encoding: "utf8" })
);
...
router.get("/", async (ctx, next) => {
  const res = await pgClient.query(
    "SELECT ts, value FROM temperatures ORDER BY ts desc LIMIT 1"
  );
  const { ts: latestTs, value: latestValue } = res.rows[0];

  const res2 = await pgClient.query(
    "SELECT ts, value FROM temperatures WHERE ts > now() - interval '1 day'"
  );
  const history = res2.rows.map((row) => ({
    time: moment(row.ts).unix(),
    value: row.value,
  }));

  const res3 = await pgClient.query(
    "SELECT ts, value FROM temperatures ORDER BY value desc LIMIT 1"
  );
  const res4 = await pgClient.query(
    "SELECT ts, value FROM temperatures ORDER BY value asc LIMIT 1"
  );
...
router.post("/temperatures.json", body(), async (ctx, next) => {
  ...
  for (const sample of data) {
    ...
    const res = await pgClient.query(
      `INSERT INTO temperatures (ts, value) VALUES ($1, $2)`,
      [date.toISOString(), value]
    );
  }

  console.log(`Received temperatures from : ${ctx.request.ip}`);
